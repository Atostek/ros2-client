...
use futures::{stream::StreamExt, FutureExt as StdFutureExt, lock::Mutex};
...
//
// This version can serve multiple concurrent clients.
...
fn main() {
  ...
  let fibonacci_action_server = Arc::new(Mutex::new(action::AsyncActionServer::new(
    node
  ...
  let main_loop = async {
    ...
    while run {
      ...
      let mut lock = fibonacci_action_server.lock().await;
      let new_goal_fut = lock.receive_new_goal().fuse();
      pin!(new_goal_fut);

      futures::select! {
        _ = stop => run = false,

        new_goal_handle = new_goal_fut => {
          match new_goal_handle {
            Ok(new_goal_handle) => {
              let fib_order = usize::try_from( *fibonacci_action_server.lock().await.get_new_goal(new_goal_handle).unwrap()).unwrap();
              info!("New goal. order={fib_order} goal_id={:?}", new_goal_handle.goal_id());
              if  !(1..=25).contains(&fib_order) {
                fibonacci_action_server.lock().await.reject_goal(new_goal_handle).await.unwrap();
                info!("Goal rejected. order={fib_order} should be between 1 and 25.");
              } else {
                // fork a child server
...
async fn action_runner(
  fibonacci_action_server: Arc<Mutex<AsyncActionServer<FibonacciAction>>>, 
  ...
  fib_order: usize ) 
{
  ...
  let accepted_goal =
    fibonacci_action_server.lock().await.accept_goal(new_goal_handle).await.unwrap();
  info!("Goal accepted. order={fib_order}");
  let executing_goal =
    fibonacci_action_server.lock().await.start_executing_goal(accepted_goal).await.unwrap();
  ...
  let result_status = loop {
      let lock = fibonacci_action_server.clone().lock_owned().await;
      let cancel_fut = lock.receive_cancel_request().fuse();
      pin!(cancel_fut);
      futures::select! {
        // _ = stop => {
        //   run = false;
